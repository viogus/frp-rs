...
/// password.
///
/// frp-rs patch: the PuTTY `.ppk` branch is removed. Upstream russh calls
/// `PrivateKey::from_ppk` here, which pulls ssh-key's `ppk` feature and its
/// pre-release argon2 + blake2 dependency chain; frp-rs never loads `.ppk`
/// keys (its SSH gateway loads only OpenSSH-format host keys), so the branch
/// is dead code. PuTTY key files now fall through to the PEM format detection
/// below and return `Error::CouldNotReadKey`.
