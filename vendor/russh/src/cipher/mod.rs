...
// frp-rs note: only the `flate2`-gated compression paths read this constant;
// with default-features off (ring+rsa, no flate2) it is dead — the allow keeps
// the vendored build warning-free (upstream is silent only because registry
// deps suppress warnings).
#[allow(dead_code)]
