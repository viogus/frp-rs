...
pub(crate) enum Action {
    ...
    Ping(Frame<Ping>),
    /// A stream should be reset: the remote opened a stream that cannot
    /// be admitted (e.g. past `Config::max_num_streams`). Send a
    /// per-stream RST and keep the connection alive — frp-rs patch
    /// mirroring Go's fatedier/yamux fork, which resets the offending
    /// stream instead of GoAway-ing the whole session.
    Reset(StreamId),
                ...
                match self.socket.poll_next_unpin(cx) {
                    Poll::Ready(Some(frame)) => {
                        match self.on_frame(frame?)? {
                            ...
                            Action::Reset(id) => {
                                // Per-stream reset: queue a Data-typed RST
                                // frame for `id` (same shape as the drop
