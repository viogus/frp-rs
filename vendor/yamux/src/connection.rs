...
use crate::{
    ...
    frame::{self, Frame, MAX_WRITE_QUEUE_FRAMES},
...
/// Maximum number of frames pulled from the stream receivers per drain step.
///
/// `SelectAll` rotates across the stream receivers, so no stream starves, and
/// the bound caps per-iteration work so a hot producer cannot pin the
/// connection task.
const MAX_FRAMES_PER_DRAIN: usize = 64;
...
impl<T: AsyncRead + AsyncWrite + Unpin> Active<T> {
    ...
    fn close(self) -> Closing<T> {
        // Frames still in the socket's write queue are flushed by `Closing`'s
        // `poll_ready`/`poll_flush` calls, so only the privileged pending
        // `Pong`/`GoAway` frame needs to be carried over here.
    ...
    fn poll(&mut self, cx: &mut Context<'_>) -> Poll<Result<Stream>> {
        loop {
            ...
            // Drain a bounded batch of frames from the stream receivers into
            // the socket's write queue, so a single `poll_write` (via the
            // batched `poll_ready` above) can carry many frames. `SelectAll`
            // rotates across the stream receivers, so no stream starves, and
            // the 64-frame bound caps the per-iteration work so a hot producer
            // cannot pin the connection task.
            //
            // Guard on `pending_read_frame` so a freshly-arrived `Pong` or
            // `GoAway` is not buried behind frames queued by this drain (stock
            // yamux guarded the same way via `pending_write_frame`). A pong can
            // still trail up to 256 frames already queued from earlier drain
            // cycles — bounded, protocol-legal (yamux mandates no
            // ping/data interleaving), and Go-frp compatible.
            let mut drained = 0;
            while self.pending_read_frame.is_none()
                && drained < MAX_FRAMES_PER_DRAIN
                && self.socket.get_ref().write_queue_len() < MAX_WRITE_QUEUE_FRAMES
            {
                match self.stream_receivers.poll_next_unpin(cx) {
                    ...
                    Poll::Ready(Some((id, Some(StreamCommand::CloseStream { ack })))) => {
                        ...
                        self.socket
                            .start_send_unpin(Frame::close_stream(id, ack).into())?;
                        drained += 1;
                    }
            ...
            // Go back and flush the batch we just queued: the last frames
            // pulled from the receivers must not sit in the write queue while
            // the task parks, or nothing would ever wake it to flush them (a
            // quiet peer produces no read event and no write waker was
            // registered).
            if drained > 0 {
                continue;
            }
