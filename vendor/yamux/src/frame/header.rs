...
impl Header<GoAway> {
    ...
    // frp-rs patch: only used by `Frame::internal_error`, whose only
    // caller (the inbound stream-cap Terminate arm) now sends a
    // per-stream Reset; kept for upstream parity.
    #[allow(dead_code)]
