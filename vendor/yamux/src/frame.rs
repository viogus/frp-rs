...
impl Frame<GoAway> {
    ...
    // frp-rs patch: the only caller (the inbound stream-cap Terminate arm
    // in connection.rs) now sends a per-stream Reset; kept for upstream
    // parity. Remove when upstream drops it.
    #[allow(dead_code)]
