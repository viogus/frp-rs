                            ...
                            tokio::spawn(async move {
                                ...
                                match frp_core::protocol::read_msg_v1(&mut ctl).await {
                                    Ok(frp_core::msg::FrpMessage::Login(login)) => {
                                        ...
                                        tokio::spawn(async move {
                                            tracing::info!("QUIC drain task started, waiting for work streams...");
                                            loop {
                                                match drain_conn.accept_bi().await {
                                                    Ok(work_stream) => {
                                                        tracing::info!("QUIC drain: accepted new work stream");
                                                    ...
                                                    Err(e) => {
                                                        tracing::info!("QUIC drain done (conn closed): {e}");
                                                        break;
                                                    }
                                                }
                                            }
