...
impl<Output: Write> Kcp<Output> {
    ...
    pub fn flush(&mut self) -> KcpResult<()> {
        ...
        // (Original C kcp checks RTO before fast_retransmit; kcp-go reordered.)
        for snd_segment in &mut self.snd_buf {
            ...
            if snd_segment.xmit == 0 {
                ...
                // ── early retransmit (kcp-go only; not in original C) ──
                need_send = true;
                snd_segment.xmit += 1;
                snd_segment.fastack = 0;
                snd_segment.rto = self.rx_rto;
                ...
                if !self.nodelay {
                    ...
                    // (original C kcp uses EXPONENTIAL: rto += rto/2)
