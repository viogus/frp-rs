...
impl KcpStream {
    ...
    pub async fn connect_with_conv(config: &KcpConfig, conv: u32, addr: SocketAddr) -> KcpResult<KcpStream> {
        ...
        KcpStream::customize_connect_with_socket_conv(config, conv, udp, addr, Vec::new()).await
    }
    ...
    //pub async fn customize_connect_with_socket

    pub async fn customize_connect_with_conv(config: &KcpConfig,conv: u32, addr: SocketAddr,salt: Vec<u8>)-> KcpResult<KcpStream>{
        let udp = match addr.ip() {
            IpAddr::V4(..) => UdpSocket::bind("0.0.0.0:0").await?,
            IpAddr::V6(..) => UdpSocket::bind("[::]:0").await?,
        };
        KcpStream::customize_connect_with_socket_conv(config, conv, udp, addr, salt).await
    }

    /// Create a `KcpStream` with an existed `UdpSocket` connecting to `addr`
    ///
    /// NOTE: `conv` will be randomly generated
    pub async fn customize_connect_with_socket(config: &KcpConfig, udp: UdpSocket, addr: SocketAddr, salt: Vec<u8>) -> KcpResult<KcpStream> {
        let mut conv = rand::random();
        while conv == 0 {
            conv = rand::random();
        }
    ...
    ///
    /// `conv` is the conversation identifier, setting to `0` will let server to randomly generate one for you.
    pub async fn customize_connect_with_socket_conv(
        config: &KcpConfig,
        conv: u32,
        udp: UdpSocket,
        addr: SocketAddr,
        ...
        let udp = Arc::new(udp);
        let socket = KcpSocket::new(config, conv, udp, addr, config.stream, salt.clone())?;
