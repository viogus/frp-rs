...
frp-rs implements ~50% of Go frp v0.69.1's feature surface. Core tunneling (TCP/UDP/HTTP/STCP/XTCP/SUDP/TCPMux) works. Gaps are mostly in operational tooling, plugins, and advanced config.
...
| HTTPS | ⚠️ Divergent | **frp-rs terminates TLS on server** (requires cert); Go frp does **SNI-only routing** without termination. Incompatible. |
...
| QUIC | ✅ | ✅ | ❌ | **ALPN mismatch**: frp-rs uses `"frp-rs"`, Go frp uses `"frp"`. Also: Go frp opens multiple streams per QUIC conn; frp-rs opens 1. |
...
| Dashboard TLS | ✅ (`dashboard_tls_cert_file`) | ❌ (plain HTTP only) |
| Pprof endpoint | ✅ | ❌ |
...
4. **SSH Tunnel Gateway** — Go frp can act as SSH bastion
5. **V2 wire protocol** — AEAD encryption, capability negotiation
6. **7 client plugins** (see table above)
7. **Visitor plugins** (virtual_net)
8. **XTCP advanced config** (Protocol, KeepTunnelOpen, MaxRetriesAnHour, MinRetryInterval, FallbackTimeoutMs, NatTraversal)
9. **Dashboard client list API**
10. **Dynamic token sourcing** (file://, exec://)
11. **Dashboard TLS**
12. **Auth fail delay** (brute-force protection)
13. **Pprof profiling endpoint**
14. **Custom 404 page**
15. **Proxy protocol version** (v1/v2 per-proxy)
