...
| KCP | ✅ | ✅ | ✅ Full interop | Window 1024, MTU 1350. Go↔Rust basic KCP passes both directions. KCP+TLS and KCP+TCPMux variants guarded — Go frp uses kcp-go session layer (FEC + XOR), Rust uses the in-tree KCP implementation (kcp-go v5.6.13 aligned). |
...
**Bottom line**: TCP, WebSocket, TLS, KCP, and QUIC are fully cross-compatible with Go frp. KCP+TLS and KCP+TCPMux variants guarded (kcp-go session layer vs in-tree Rust KCP).
...
- ✅ KCP cross-compat: Rust↔Rust KCP + Go↔Rust basic KCP both pass in both directions. KCP+TLS and KCP+TCPMux guarded — wire format mismatch (Go kcp-go session layer with FEC+XOR vs Rust in-tree KCP)
