...
The measurement is the table in
[README § Technical Differences vs Go frp](../README.md#technical-differences-vs-go-frp),
generated by `bash scripts/compare-go-frp.sh --build --memory`. It is deliberately
**not copied here**: it used to exist in two places, the copies drifted, and the wrong
one always looked authoritative. In short — **3.5× smaller and 2.7× lighter** than Go
frp at the default tier, and up to **9× smaller** at `micro`.

> The script downloads the official Go release for *this* platform and version and
> **aborts if either does not match** — a cross-platform or cross-version comparison
> is not a measurement. Absolute figures are platform-dependent: re-run it on your
> target platform instead of trusting any published numbers. CI builds override
> LTO/opt (`lto=false opt-level=2`) for speed and are larger, so CI artifacts do not
> reflect a release.
...
Idle RSS is about a third of Go frp's at the default tier — the same generated table
has the figures. More importantly, Rust has no garbage collector: the heap cannot
