...
<!-- Generated by `bash scripts/compare-go-frp.sh --build --memory` on 2026-09-17 —
     platform: darwin_arm64; frp-rs 0.71.0; Go frp 0.71.0 (official release);
     rustc 1.96.0; declared release profile. Regenerate rather than editing by hand:
     the previous hand-written version of this table compared macOS Go binaries
     against Linux frp-rs numbers, so none of it was reproducible. -->
...
That is **3.5× smaller binaries and 2.7× less idle RSS** for frps at the default
tier, and up to **9× smaller** at the `micro` tier.

> Measured on **macOS arm64** with the **declared release profile** (`fat-LTO`,
> `opt-level=z`, `codegen-units=1`, `strip = "symbols"`, `panic=abort`), using
> `scripts/compare-go-frp.sh`, which downloads the official Go release for *this*
> platform and version and **aborts if either does not match** — a cross-platform
> or cross-version comparison is not a measurement. Absolute sizes and RSS are
> platform-dependent; re-run the script on your target platform rather than
> trusting these numbers. CI builds override LTO/opt (`lto=false opt-level=2`) for
> speed and come out larger, so CI artifacts do not reflect release.

The `tiny`/`micro` rows are also the deployment argument: they let frpc run on
devices where a 17 MB Go binary would not fit at all. Idle RSS for those tiers is
not yet measured by the script — see
[`TODO.md`](TODO.md).
