...
fn init_logging(cli: &FrpcRunArgs, cfg: Option<&ClientConfig>) {
    let level = logging::resolve_log_level(cli.log_level.clone(), cfg.map(|c| c.log.level.as_str()), "debug,yamux=trace");
    ...
    #[cfg(feature = "otel")]
    {
        ...
        let svc_name = cfg.and_then(|c| if c.observability.service_name.is_empty() { None } else { Some(c.observability.service_name.clone()) }).unwrap_or_else(|| "frpc".to_string());
