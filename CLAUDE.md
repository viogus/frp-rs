...
words, or added to a new doc, is not caught until its entry is added.
...
- **Property/fuzz tests**: proptest-based config normalization (`frp-core/src/config/tests.rs`) and V1/V2 protocol frame fuzzing (`frp-core/src/protocol.rs`, 6 fuzz tests, 0 panics found). The fuzz-target count is cross-checked by `repo-health.sh`; the raw test totals are volatile and printed by the script rather than stored here.
- **Integration tests**: KCP real-UDP-socket test (`frp-core/tests/kcp.rs`), XTCP hole-punch e2e (`frp-server/tests/xtcp_hole_punch.rs`), plus a growing server-integration suite covering control handler, vhost, proxy registration, OIDC, reload, graceful drain — the count is volatile and is printed by `repo-health.sh` rather than stored here.
...
- **Repo invariants**: `scripts/repo-health.sh` is the local mirror of the `health` CI job — it gates version alignment and that every **backtick-delimited** repo path anchored at a known root in current docs or source comments resolves (file-relative, then owning crate root, then repo root; `crate/feature` recognised as Cargo feature syntax). Not covered, by design: un-backticked prose and tree diagrams, locator-less shorthands (`mux.rs`), a directory that still exists but was emptied, and third-party `vendor/*/README.md`. Point-in-time files (`CHANGELOG.md`, `TODO.md`, `performance-audit.md`, `docs/refactor-large-modules.md`) are out of scope. See [docs/developing.md § Repository Invariants](docs/developing.md#repository-invariants-repo-healthsh).
