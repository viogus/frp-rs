...
| `cargo test --workspace --all-features` | must pass — needs an all-features `frps` binary, see Testing & Tooling. **The pass count is a runtime fact; do not quote a stored number here.** 2069 test functions exist in-tree and are re-counted by `repo-health.sh` ("Tests"), which fails if this figure drifts |
...
- **Property/fuzz tests**: proptest-based config normalization (`frp-core/src/config/tests.rs`, 11 proptest! blocks) and V1/V2 protocol frame fuzzing (`frp-core/src/protocol.rs`, 6 fuzz tests + 40 regular tests, 0 panics found). Both counts are cross-checked by `repo-health.sh`.
- **Integration tests**: KCP real-UDP-socket test (`frp-core/tests/kcp.rs`), XTCP hole-punch e2e (`frp-server/tests/xtcp_hole_punch.rs`), plus 215+ server integration tests covering control handler, vhost, proxy registration, OIDC, reload, graceful drain (counted by `repo-health.sh`).
...
- **Repo invariants**: `scripts/repo-health.sh` is the local mirror of the `health` CI job — it gates version alignment and that every repo path named in current docs or source comments resolves (file-relative, then repo root; `crate/feature` recognised as Cargo feature syntax). Locator-less shorthands and "directory exists but emptied" are not checked. See [docs/developing.md § Repository Invariants](docs/developing.md#repository-invariants-repo-healthsh).
