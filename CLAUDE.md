...
- **Repo invariants**: `scripts/repo-health.sh` is the local mirror of the `health` CI job — it gates version alignment and that every **backtick-delimited** repo path anchored at a known root in current docs or source comments resolves (file-relative, then owning crate root, then repo root; `crate/feature` recognised as Cargo feature syntax). The path scan is **tracked-files-only** (file list from `git ls-files -z`; a `.git`-less tree falls back to walking the filesystem), so gitignored local state such as `.worktrees/` and `.superpowers/` is never scanned. Not covered, by design: un-backticked prose and tree diagrams, locator-less shorthands (`mux.rs`), a directory that still exists but was emptied, untracked local files, and third-party `vendor/*/README.md`. Point-in-time files (`CHANGELOG.md`, `TODO.md`, `performance-audit.md`, `docs/refactor-large-modules.md`) are out of scope. See [docs/developing.md § Repository Invariants](docs/developing.md#repository-invariants-repo-healthsh).
